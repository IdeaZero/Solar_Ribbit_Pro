...
int main(void)
{
	...
	rc = gpio_pin_configure_dt(&boost_5v, GPIO_OUTPUT_ACTIVE);
	if (rc < 0)
	{
		LOG_ERR("Error configuring boost 5v (err %d)", rc);
		// return; // Or handle the error appropriately
	}
	...
	rc = gpio_pin_configure_dt(&boost_3v3, GPIO_OUTPUT_ACTIVE);
	if (rc < 0)
	{
		LOG_ERR("Error configuring boost 3v3 (err %d)", rc);
		// return; // Or handle the error appropriately
	}
